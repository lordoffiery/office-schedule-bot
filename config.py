...
# Максимальное количество мест в офисе (все места в первом ряду: 1.1 … 1.11)
MAX_OFFICE_SEATS = 11

# Расписание по умолчанию (JSON формат: день -> {подразделение.место: имя})
# Формат ключа: "подразделение.место" (например, "1.1", "1.2"; сейчас используется только ряд 1.x)
