...
class ScheduleManager:
    ...
    def sync_requests_from_schedules(self, week_start: datetime, employee_manager) -> Dict[str, int]:
        """
        Восстановить заявки (requests) из расписания (schedules) для будущей недели
        
        Сравнивает текущее расписание с default_schedule и определяет:
        - days_requested: дни, которые есть в schedules, но нет в default_schedule
        - days_skipped: дни, которые есть в default_schedule, но нет в schedules
        
        Args:
            week_start: Начало недели
            employee_manager: Менеджер сотрудников для получения telegram_id
            
        Returns:
            Dict[str, int]: Статистика синхронизации {'synced': количество, 'errors': количество}
        """
        ...
        if not USE_POSTGRESQL:
            logger.warning("PostgreSQL недоступен, синхронизация requests невозможна")
            return stats
        ...
        try:
            ...
            # Собираем расписание на неделю из schedules
            week_schedule = {}  # {day_name: [имена сотрудников]}
            for date, day_name in week_dates:
                date_str = date.strftime('%Y-%m-%d')
                schedule_for_date = self.load_schedule_for_date(date, employee_manager)
                if day_name in schedule_for_date:
                    # Извлекаем простые имена из отформатированных
                    employees = schedule_for_date[day_name]
                    plain_names = []
                    for emp in employees:
                        plain_name = self.get_plain_name_from_formatted(emp)
                        if plain_name:
                            plain_names.append(plain_name)
                    week_schedule[day_name] = plain_names
            
            # Собираем расписание по умолчанию для недели
            default_week_schedule = {}  # {day_name: [имена сотрудников]}
            for day_name, places_dict in default_schedule.items():
                ...
                for place_key, name in places_dict.items():
                    if name:
                        plain_name = self.get_plain_name_from_formatted(name)
                        if plain_name:
                            employees.append(plain_name)
                default_week_schedule[day_name] = employees
            
            # Группируем по сотрудникам
            employees_in_schedule = set()
            for day_name, employees in week_schedule.items():
                employees_in_schedule.update(employees)
            for day_name, employees in default_week_schedule.items():
                employees_in_schedule.update(employees)
            
            # Для каждого сотрудника определяем days_requested и days_skipped
            for employee_name in employees_in_schedule:
                try:
                    # Получаем telegram_id сотрудника
                    telegram_id = employee_manager.get_employee_id(employee_name)
                    if not telegram_id:
                        logger.warning(f"Не найден telegram_id для сотрудника {employee_name}, пропускаем")
                        stats['errors'] += 1
                        continue
                    ...
                    for day_name in ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница']:
                        in_default = employee_name in default_week_schedule.get(day_name, [])
                        in_schedule = employee_name in week_schedule.get(day_name, [])
                        
                        if in_schedule and not in_default:
                            # Есть в расписании, но нет в default - запрошен дополнительно
                            days_requested.append(day_name)
                        elif in_default and not in_schedule:
                            # Есть в default, но нет в расписании - пропущен
                            days_skipped.append(day_name)
                        # Если и в default, и в schedule - это гарантированный день, не добавляем никуда
                    
                    # Сохраняем заявку
                    self.save_request(employee_name, telegram_id, week_start, days_requested, days_skipped)
                    stats['synced'] += 1
                    logger.debug(f"Восстановлена заявка для {employee_name} (неделя {week_start.strftime('%Y-%m-%d')}): requested={days_requested}, skipped={days_skipped}")
                except Exception as e:
                    logger.error(f"Ошибка восстановления заявки для {employee_name}: {e}", exc_info=True)
                    stats['errors'] += 1
            
            return stats
        except Exception as e:
            logger.error(f"Ошибка синхронизации requests из schedules для недели {week_start.strftime('%Y-%m-%d')}: {e}", exc_info=True)
            stats['errors'] += 1
            return stats
