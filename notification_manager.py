...
class NotificationManager:
    ...
    async def send_weekly_schedule(self, admins_only: bool = False):
        ...
        logger.info(f"💾 Сохранение финального расписания на неделю {next_week_start.strftime('%Y-%m-%d')} (все дни, включая совпадающие с default)")
        ...
        logger.info(f"✅ Финальное расписание сохранено в schedules (все дни недели)")
        ...
        # НЕ очищаем заявки (requests) после рассылки - они остаются для истории и аудита
        # Расписание уже сохранено в schedules, поэтому requests больше не используются для построения расписания
        # но могут быть полезны для анализа и отладки
        logger.info(f"ℹ️ Заявки (requests) на неделю {next_week_start.strftime('%Y-%m-%d')} сохранены для истории (не очищаются)")
